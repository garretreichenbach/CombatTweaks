		...
		StarLoader.registerListener(SegmentPieceDamageEvent.class, new Listener<SegmentPieceDamageEvent>() {
			...
			public void onEvent(SegmentPieceDamageEvent event) {
				try {
						...
						if(manager != null) {
							...
							if(armorHP > 0) {
								switch(event.getDamageType()) {
									case BEAM:
										damage *= (int) ConfigManager.getSystemConfig().getDouble("beam-armor-multiplier");
										break;
									case PROJECTILE:
										damage *= (int) ConfigManager.getSystemConfig().getDouble("cannon-armor-multiplier");
										break;
									case MISSILE:
										damage *= (int) ConfigManager.getSystemConfig().getDouble("missile-armor-multiplier");
										break;
								}
								for(EffectConfigElement element : manager.activeArmorEffects) {
									if(element.getType() == StatusEffectType.ARMOR_HP_ABSORPTION) {
										damage *= (int) (1 - element.getFloatValue());
					...
					CombatTweaks.getInstance().logException("Error handling block damage event", exception);
				}
			}
		}, instance);
		StarLoader.registerListener(SegmentPieceKillEvent.class, new Listener<SegmentPieceKillEvent>() {
			...
			public void onEvent(SegmentPieceKillEvent event) {
				try {
					...
					CombatTweaks.getInstance().logException("Error handling block kill event", exception);
				}
			}
		}, instance);
		/*
		StarLoader.registerListener(MissileHitEvent.class, new Listener<MissileHitEvent>() {
			...
			public void onEvent(MissileHitEvent event) {
				try {
					...
					CombatTweaks.getInstance().logException("Error handling missile hit event", exception);
				}
			}
		}, instance);
		 */
