...
public class SendMoveToPacket extends Packet {
	...
	public SendMoveToPacket(Ship ship, SegmentController target, boolean queue) {
		shipId = ship.getId();
	...
	public void processPacketOnServer(PlayerState playerState) {
		...
		if(queue) {
			OrderQueueManager.getInstance().enqueue(shipId, OrderQueueManager.OrderType.MOVE, targetId);
		} else {
			OrderQueueManager.getInstance().replace(shipId, OrderQueueManager.OrderType.MOVE, targetId);
		}
	}
}
